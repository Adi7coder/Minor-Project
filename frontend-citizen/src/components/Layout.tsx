    ...
    <div className="flex flex-col min-h-screen font-sans">
      <header className="sticky top-0 z-50 glass border-b border-white/50">
        <div className="max-w-screen-md mx-auto px-4 h-16 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2.5 text-slate-800 font-extrabold text-xl tracking-tight hover:opacity-80 transition-opacity">
            ...
            Clean<span className="text-indigo-600">BLR</span>
          </Link>
      ...
      <nav className="sm:hidden fixed bottom-6 left-4 right-4 glass-card z-50 rounded-[2rem] px-2 py-2 border-white/60">
        <div className="flex justify-around items-center h-14 relative">
          <Link 
            to="/" 
            className={`flex flex-col items-center justify-center w-full h-full rounded-2xl transition-all duration-300 ${isActive('/') ? 'text-indigo-600 bg-indigo-50/50' : 'text-slate-500 hover:text-slate-800'}`}
          >
            <div className={`transition-transform duration-300 ${isActive('/') ? '-translate-y-1' : ''}`}>
              <Home size={22} className={isActive('/') ? 'fill-indigo-100 stroke-indigo-600 stroke-2' : 'stroke-[1.5px]'} />
            </div>
          ...
          <div className="relative -top-6 flex justify-center w-full">
            <Link 
              to="/submit" 
              className="bg-gradient-to-tr from-indigo-500 to-purple-600 text-white p-4 rounded-full shadow-lg shadow-indigo-500/30 hover:shadow-xl hover:-translate-y-1 hover:scale-105 transition-all duration-300 ring-4 ring-white/50"
            >
          ...
          <Link 
            to="/track" 
            className={`flex flex-col items-center justify-center w-full h-full rounded-2xl transition-all duration-300 ${isActive('/track') ? 'text-indigo-600 bg-indigo-50/50' : 'text-slate-500 hover:text-slate-800'}`}
          >
            <div className={`transition-transform duration-300 ${isActive('/track') ? '-translate-y-1' : ''}`}>
              <Search size={22} className={isActive('/track') ? 'stroke-indigo-600 stroke-[2.5px]' : 'stroke-[1.5px]'} />
            </div>
